...
impl<T: 'static, const ADDRSPACE: u32> AddrspacePtr<T, ADDRSPACE> {
    ...
    pub fn cast<U: 'static>(self) -> AddrspacePtr<U, ADDRSPACE> {
        // SAFETY: Only the pointed-to type is converted, making this a no-op except for the type
        // conversion.
    ...
    pub unsafe fn cast_addrspace<const TARGET_ADDRSPACE: u32>(
        ...
        // SAFETY: the safety contract for `addrspace_ptr_cast` must be
        // upheld by the caller.
        unsafe { crate::intrinsics::addrspace_ptr_cast(self) }
    }
    ...
    pub unsafe fn offset(self, count: isize) -> Self {
        // We cannot convert the addrspace ptr to an integer as we do not know its size,
