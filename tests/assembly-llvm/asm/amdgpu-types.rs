...
//@[gfx11] compile-flags: --target amdgcn-amd-amdhsa -Ctarget-cpu=gfx1100
//@[gfx12] compile-flags: --target amdgcn-amd-amdhsa -Ctarget-cpu=gfx1200
//@ needs-llvm-components: amdgpu

#![feature(abi_gpu_kernel, no_core, asm_experimental_arch, f16, f128)]
#![crate_type = "rlib"]
#![no_core]
...
macro_rules! check {
    ($func:ident $ty:ident $class:ident $mov:literal) => {
        #[no_mangle]
        pub unsafe extern "gpu-kernel" fn $func(x: $ty) -> $ty {
            let y;
    ...
    ($func:ident $ret_ty:ident $ret_class:ident $($arg_name:ident: $arg_ty:ident $arg_class:ident,)* $mov:literal) => {
        #[no_mangle]
        pub unsafe extern "gpu-kernel" fn $func($($arg_name: $arg_ty,)*) -> $ret_ty {
            let result;
            asm!(concat!($mov, " {}", $(", {", stringify!($arg_name), "}",)*), out($ret_class) result, $($arg_name = in($arg_class) $arg_name,)*);
            result
        }
    };
}
...
macro_rules! check_reg {
    ...
    ($func:ident $ret_ty:ident $ret_reg:tt $($arg_name:ident: $arg_ty:ident $arg_reg:tt,)* $mov:literal) => {
        #[no_mangle]
        pub unsafe extern "gpu-kernel" fn $func($($arg_name: $arg_ty,)*) -> $ret_ty {
            let result;
            asm!(concat!($mov, " ", $ret_reg, $(", ", $arg_reg,)*), lateout($ret_reg) result, $(in($arg_reg) $arg_name,)*);
            result
        }
    };
}
